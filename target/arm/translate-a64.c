...
void a64_translate_init(void)
{
    ...
    cpu_pc = tcg_global_mem_new(cpu_env,
                                offsetof(CPUARMState, pc._cr_cursor), "pc");
    cpu_capreg_state = tcg_global_mem_new(
            cpu_env, offsetof(CPUARMState, gpcapregs.capreg_state),
            "capreg_state");
...
#define GET_FLAG(C, F) \
    (C->base.cheri_flags & (1 << (R_TBFLAG_CHERI_ ## F ## _SHIFT + TB_FLAG_CHERI_SPARE_INDEX_START)))


#define IS_C64(ctx) !!GET_FLAG(ctx, PSTATE_C64)

static inline TCGv_cap_checked_ptr gen_bounds_and_perms_checks(DisasContext* ctx, int regnum, TCGv addr, target_ulong size, bool load, bool store) {
    ...
    /*
    // TODO: finish this below and then dont use the helper
    gen_lazy_cap_decompress(ctx, regnum);

    size_t base_off = gp_register_offset(regnum)
    ...
    // addr >= base
    tcg_gen_ld_tl(t1, cpu_env, base_off);
    tcg_gen_setcond_tl(TCG_COND_GEU, cond, addr, t1);

    // addr+size <= top
    tcg_gen_ld_tl(t1, cpu_env, top_off);
    tcg_gen_andi_tl(t2, addr, size);
    tcg_gen_setcond_tl(TCG_COND_LEU, t2, t2, t1);
    if(sizeof(((cap_register_t*)(0))->_cr_top) == 16) {
        // If the top bit is set, then obviously we are below top
        tcg_gen_ld_tl(t1, cpu_env, top_off+8);
        // do I need a not equal zero? I assume the lowest bit is the only thing that could ever be set
        tcg_gen_or_tl(t2, t2, t1);
    }
    tcg_gen_and_tl(cond, cond, t2);

    // perms, tagged, & not sealed LETODO
    tcg_gen_movi_i64(t1, 0);



    // if cond != 0, branch to label
    tcg_gen_brcond_i64(TCG_COND_NE, cond, t1, l1);

    // Throw an exception LETODO

    // Label l1 (all checks pass)
    gen_set_label(l1);
...
static TCGv_cap_checked_ptr arm_bounds_checked(DisasContext *s, TCGv_i64 tcg_addr, int size, int base_reg,
        bool is_load, bool alternate_base, bool against_ddc) {
    bool capability_base = against_ddc && (IS_C64(s) != alternate_base);
    if(capability_base) {
        return gen_bounds_and_perms_checks(s, base_reg, tcg_addr, size, is_load, !is_load);
    } else {
        _generate_special_checked_ptr(s, is_load ? FLAG_READABLE(against_ddc) : FLAG_WRITABLE(against_ddc),
...
#else
#define arm_bounds_checked(s, tcg_addr, size, base_reg, is_load, alternate_base, against_ddc) tcg_addr
#define IS_C64(ctx) false
#endif

// Untag / zero top of register as appropriate
static void incremented_register_base(DisasContext *s, int regnum, bool capability_base) {
#ifdef TARGET_CHERI
    if(capability_base) {
        gen_cap_modified(s, regnum);
    } else {
...
static void incremented_register(DisasContext *s, int regnum) {
    incremented_register_base(s, regnum, IS_C64(s));
}
...
TCGv_cap_checked_ptr clean_data_tbi_and_cheri(DisasContext *s, TCGv_i64 addr, bool is_load, int size, int base_reg,
        bool alternate_base, bool ddc_base)
{
    TCGv_i64 clean = clean_data_tbi(s, addr);
    return arm_bounds_checked(s, clean, size, base_reg, is_load, alternate_base, ddc_base);
}
 ...
 */
static TCGv_cap_checked_ptr gen_mte_and_cheri_check1_mmuidx(DisasContext *s, TCGv_i64 addr,
                                      bool is_write, bool tag_checked,
                                      int log2_size, bool is_unpriv,
                                      int core_idx,
                                      int base_reg, bool alternate_base, bool ddc_base)
{
    if (tag_checked && s->mte_active[is_unpriv]) {
        ...
        return arm_bounds_checked(s, ret, (1 << log2_size), base_reg, !is_write, alternate_base, ddc_base);
    }
    return clean_data_tbi_and_cheri(s, addr, !is_write, 1 << log2_size, base_reg, alternate_base, ddc_base);
}

TCGv_cap_checked_ptr gen_mte_and_cheri_check1(DisasContext *s, TCGv_i64 addr, bool is_write,
                        bool tag_checked, int log2_size, int base_reg, bool alternate_base, bool ddc_base)
{
    return gen_mte_and_cheri_check1_mmuidx(s, addr, is_write, tag_checked, log2_size,
                                 false, get_mem_index(s), base_reg, alternate_base, ddc_base);
}

/*
 * For MTE, check multiple logical sequential accesses.
 */
TCGv_cap_checked_ptr gen_mte_and_cheri_checkN(DisasContext *s, TCGv_i64 addr, bool is_write,
                        ...
                        int base_reg, bool alternate_base, bool ddc_base)
{
    if (tag_checked && s->mte_active[0] && total_size != (1 << log2_esize)) {
        ...
        return arm_bounds_checked(s, ret, total_size, base_reg, !is_write, alternate_base, ddc_base);
    }
    return gen_mte_and_cheri_check1(s, addr, is_write, tag_checked, log2_esize, base_reg, alternate_base, ddc_base);
}
...
static void do_gpr_st_memidx(DisasContext *s, TCGv_i64 source,
                             TCGv_cap_checked_ptr tcg_addr, int size, int memidx,
                             bool iss_valid,
                             unsigned int iss_srt,
                             bool iss_sf, bool iss_ar)
{
    g_assert(size <= 3);
    tcg_gen_qemu_st_i64_with_checked_addr(source, tcg_addr, memidx, s->be_data + size);
...
static void do_gpr_ld_memidx(DisasContext *s,
                             TCGv_i64 dest, TCGv_cap_checked_ptr tcg_addr,
 ...
 */
static void do_fp_st(DisasContext *s, int srcidx, TCGv_cap_checked_ptr tcg_addr, int size)
{
    ...
    if (size < 4) {
        ...
        tcg_gen_qemu_st_i64_with_checked_addr(tmp, be ? tcg_hiaddr : tcg_addr, get_mem_index(s),
                            s->be_data | MO_Q);
        tcg_gen_ld_i64(tmp, cpu_env, fp_reg_hi_offset(s, srcidx));
        tcg_gen_qemu_st_i64_with_checked_addr(tmp, be ? tcg_addr : tcg_hiaddr, get_mem_index(s),
                            s->be_data | MO_Q);
...
static void do_fp_ld(DisasContext *s, int destidx, TCGv_cap_checked_ptr tcg_addr, int size)
{
    ...
    if (size < 4) {
        MemOp memop = s->be_data + size;
        tcg_gen_qemu_ld_i64_with_checked_addr(tmplo, tcg_addr, get_mem_index(s), memop);
    } else {
        bool be = s->be_data == MO_BE;
        ...
        tcg_gen_addi_i64((TCGv_i64)tcg_hiaddr, (TCGv_i64)tcg_addr, 8);
        tcg_gen_qemu_ld_i64_with_checked_addr(tmplo, be ? tcg_hiaddr : tcg_addr, get_mem_index(s),
                            s->be_data | MO_Q);
        tcg_gen_qemu_ld_i64_with_checked_addr(tmphi, be ? tcg_addr : tcg_hiaddr, get_mem_index(s),
                            s->be_data | MO_Q);
        tcg_temp_free_cap_checked(tcg_hiaddr);
    }
                              ...
                              TCGv_cap_checked_ptr tcg_addr, int size, MemOp endian)
{
    ...
    tcg_gen_qemu_st_i64_with_checked_addr(tcg_tmp, tcg_addr, get_mem_index(s), endian | size);
                              ...
                              TCGv_cap_checked_ptr tcg_addr, int size, MemOp endian)
{
    ...
    tcg_gen_qemu_ld_i64_with_checked_addr(tcg_tmp, tcg_addr, get_mem_index(s), endian | size);
                       ...
                       unsigned int crn, unsigned int crm, unsigned int rt)
{
    ...
    if ((ri->access & PL_SYSREG) && !cheri_is_system_ctx(s)) {
        assert(0); // should throw code 0x18
    }
#endif
    ...
    // LETODO would be nice to log the modifcation to the CSR
#endif
    ...
    if (is_morello) {
        // Morello instructions might be able to modify non-cap registers. But I don't know of any.
...
static void gen_load_exclusive(DisasContext *s, int rt, int rt2,
                               TCGv_cap_checked_ptr addr, int size, bool is_pair)
{
    ...
    if (is_pair) {
        ...
        if (size == 2) {
            ...
            tcg_gen_qemu_ld_i64_with_checked_addr(cpu_exclusive_val, addr, idx, memop);
            if (s->be_data == MO_LE) {
            ...
            tcg_gen_qemu_ld_i64_with_checked_addr(cpu_exclusive_high, addr2, idx, memop);
...
static void gen_store_exclusive(DisasContext *s, int rd, int rt, int rt2,
                                TCGv_cap_checked_ptr addr, int size, int is_pair)
{
    ...
    tcg_gen_brcond_i64(TCG_COND_NE, (TCGv_i64)addr, cpu_exclusive_addr, fail_label);
    ...
    if (is_pair) {
        if (size == 2) {
            ...
            tcg_gen_atomic_cmpxchg_i64_with_checked_addr(tmp, (TCGv_cap_checked_ptr)cpu_exclusive_addr,
        ...
        tcg_gen_atomic_cmpxchg_i64_with_checked_addr(tmp, (TCGv_cap_checked_ptr)cpu_exclusive_addr, cpu_exclusive_val,
                                 ...
                                 int rn, int size)
{
    ...
    clean_addr = gen_mte_and_cheri_check1(s, cpu_reg_sp(s, rn), true, rn != 31, size, rn, false, true);
    tcg_gen_atomic_cmpxchg_i64_with_checked_addr(tcg_rs, clean_addr, tcg_rs, tcg_rt, memidx,
                              size | MO_ALIGN | s->be_data);
}
                                      ...
                                      int rn, int size)
{
    ...
    clean_addr = gen_mte_and_cheri_check1(s, cpu_reg_sp(s, rn), true, rn != 31, size + 1, rn, false, true);
    ...
    if (size == 2) {
        ...
        tcg_gen_atomic_cmpxchg_i64_with_checked_addr(cmp, clean_addr, cmp, val, memidx,
                                  MO_64 | MO_ALIGN | s->be_data);
        ...
        tcg_gen_qemu_ld_i64_with_checked_addr(d2, (TCGv_cap_checked_ptr)a2, memidx, MO_64 | s->be_data);
        ...
        tcg_gen_qemu_st_i64_with_checked_addr(c1, clean_addr, memidx, MO_64 | s->be_data);
        tcg_gen_qemu_st_i64_with_checked_addr(c2, (TCGv_cap_checked_ptr)a2, memidx, MO_64 | s->be_data);
...
static void disas_ldst_excl(DisasContext *s, uint32_t insn)
{
    ...
    case 0x1: /* STLXR */
        ...
        clean_addr = gen_mte_and_cheri_check1(s, cpu_reg_sp(s, rn),
                                    true, rn != 31, size, rn, false, true);
    ...
    case 0x5: /* LDAXR */
        ...
        clean_addr = gen_mte_and_cheri_check1(s, cpu_reg_sp(s, rn),
                                    false, rn != 31, size, rn, false, true);
        s->is_ldex = true;
...
static void disas_ld_lit(DisasContext *s, uint32_t insn)
{
    ...
    clean_addr = arm_bounds_checked(s, dirty_addr, 1 << size, 0, true, false, false);
...
static void disas_ldst_pair(DisasContext *s, uint32_t insn)
{
    ...
    clean_addr = gen_mte_and_cheri_checkN(s, dirty_addr, !is_load,
                                (wback || rn != 31) && !set_tag,
                                size, 2 << size, rn, false, true);
    ...
    if (is_vector) {
        ...
        if (is_load) {
            ...
            do_gpr_ld(s, tmp, clean_addr, size, is_signed, false,
                      false, (unsigned int)-1, false, false);
            tcg_gen_addi_i64((TCGv_i64)clean_addr, (TCGv_i64)clean_addr, 1 << size);
                                ...
                                bool is_vector)
{
    ...
    clean_addr = gen_mte_and_cheri_check1_mmuidx(s, dirty_addr, is_store,
                                       writeback || rn != 31,
                                       size, is_unpriv, memidx, rn, false, true);
                                   ...
                                   bool is_vector)
{
    ...
    clean_addr = gen_mte_and_cheri_check1(s, dirty_addr, is_store, true, size, rn, false, true);
                                        ...
                                        bool is_vector)
{
    ...
    offset = imm12 << size;
    tcg_gen_addi_i64(dirty_addr, dirty_addr, offset);
    clean_addr = gen_mte_and_cheri_check1(s, dirty_addr, is_store, rn != 31, size, rn, false, true);
                              ...
                              int size, int rt, bool is_vector)
{
    ...
    clean_addr = gen_mte_and_cheri_check1(s, cpu_reg_sp(s, rn), false, rn != 31, size, rn, false, true);
                           ...
                           int size, int rt, bool is_vector)
{
    ...
    clean_addr = gen_mte_and_cheri_check1(s, dirty_addr, false,
...
static void disas_ldst_ldapr_stlr(DisasContext *s, uint32_t insn)
{
    ...
    dirty_addr = read_cpu_reg_sp(s, rn, 1);
    tcg_gen_addi_i64(dirty_addr, dirty_addr, offset);
    clean_addr = clean_data_tbi_and_cheri(s, dirty_addr, !is_store, 1 << size, rn, false, true);
...
static void disas_ldst_multiple_struct(DisasContext *s, uint32_t insn)
{
     ...
     */
    clean_addr = gen_mte_and_cheri_checkN(s, tcg_rn, is_store, is_postidx || rn != 31,
                                size, total, rn, false, true);
...
static void disas_ldst_single_struct(DisasContext *s, uint32_t insn)
{
    ...
    clean_addr = gen_mte_and_cheri_checkN(s, tcg_rn, !is_load, is_postidx || rn != 31,
                                scale, total, rn, false, true);
...
static void disas_ldst_tag(DisasContext *s, uint32_t insn)
{
    ...
    if (is_zero) {
        ...
        tcg_gen_qemu_st_i64_with_checked_addr(tcg_zero, (TCGv_cap_checked_ptr)clean_addr, mem_index,
                            MO_Q | MO_ALIGN_16);
        for (i = 8; i < n; i += 8) {
            tcg_gen_addi_i64(clean_addr, clean_addr, 8);
            tcg_gen_qemu_st_i64_with_checked_addr(tcg_zero, (TCGv_cap_checked_ptr)clean_addr, mem_index, MO_Q);
        }
        tcg_temp_free_i64(tcg_zero);
    }
