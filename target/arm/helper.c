...
void register_cp_regs_for_features(ARMCPU *cpu)
{
    ...
    ARMCPRegInfo cheri_regs[] = {
            ...
            {   .name = "CHCR_EL2",
                ...
                .resetvalue = 0},
            {   .name = "CSCR_EL3",
                .opc0 = 3, .opc1 = 5, .crn = 1, .crm = 2, .opc2 = 3,
                .access =  PL3_RW, .type = 0, .state = ARM_CP_STATE_AA64,
...
#endif
        set_aarch_reg_to_x(&env->elr_el[new_el], get_aarch_reg_as_x(&env->pc));
    } else {
        ASSERT_IF_CHERI();
#ifndef TARGET_CHERI
...
static inline uint64_t regime_cctlr(CPUARMState *env, ARMMMUIdx mmu_idx) {
    // Note: vttbr_el2 really does not correspond to CCTLR_el2. But these bits will get masked out anyway.
    return env->CCTLR_el[mmu_idx == ARMMMUIdx_Stage2 ? 2 : regime_el(env, mmu_idx)];
}
#endif
...
#ifdef TARGET_CHERI
static inline uint32_t aa64_effective_hwu(CPUARMState *env, ARMMMUIdx mmu_idx, ARMVAParameters* params, uint64_t tcr) {
    if (!params->hpd)
        return 0;
 ...
 * @access_type: MMU_DATA_LOAD, MMU_DATA_STORE or MMU_INST_FETCH (or MMU_DATA_CAP_LOAD, MMU_DATA_CAP_STORE on CHERI)
 * @mmu_idx: MMU index indicating required translation regime
                               ...
                               ARMMMUFaultInfo *fi, ARMCacheAttrs *cacheattrs)
{
    ...
    // Cap stores can fault here as only tagged stores specify MMU_DATA_CAP_STORE
    if (access_type == MMU_DATA_CAP_STORE) {
        if (!cdbm && !sc) goto do_fault;
        access_type = MMU_DATA_STORE;
    }

    // Cap loads just need normal load permission at this point, because traps / clears require the tag
...
static inline void assert_hflags_rebuild_correctly(CPUARMState *env)
{
    ...
    if(unlikely(cheri_flags != env->chflags)) {
        fprintf(stderr, "TCG cheriflags mismatch (current:0x%08x rebuilt:0x%08x)\n",
...
#ifdef TARGET_CHERI
        cheri_cpu_get_tb_cpu_state(&env->pc, cheri_get_ddc(env),
                                          ...
                                          uintptr_t retpc)
{
    ...
    *prot = 0;
    ...
    bool fail = get_phys_addr(env, address, rw, idx, &physical,
                              &attrs, prot, &page_size, &fi, &cacheattrs);

    if(fail) {
        printf("prot: %x. fi type %d (perms is %d)\n", *prot, fi.type, ARMFault_Permission);
...
void HELPER(arm_log_instr)(CPUARMState *env, target_ulong pc,
                             uint32_t opcode)
{
