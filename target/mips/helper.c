...
{
    uint64_t tag;
    /* XXX MMU_DATA_CAP_LOAD?  Causes some bizarre misbehavior on CheriBSD
     * if so, tho'.  Needs further debugging.
     */
    uint8_t *tagblk = cheri_tag_get_block(env, vaddr, MMU_DATA_LOAD, reg,
        ...
        uint8_t tagbit, uint64_t tps, uint64_t length, uintptr_t pc)
{
    ...
    uint64_t *tagblk64;
    ram_addr_t ram_addr;
    uint8_t *tagblk = cheri_tag_get_block(env, vaddr, MMU_DATA_CAP_STORE, reg,
                                          0, pc, NULL, &ram_addr, &tag);
    if (tagblk == NULL) {
        /* Allocated a tag block. */
        tagblk = cheri_tag_new_tagblk(tag);
    }
