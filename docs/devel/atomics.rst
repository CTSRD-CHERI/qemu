...
- weak atomic access and manual memory barriers: ``qatomic_read()``,
  ``qatomic_set()``, ``smp_rmb()``, ``smp_wmb()``, ``smp_mb()``, ``smp_mb_acquire()``,
...
- Originally, ``qatomic_read`` and ``qatomic_set`` in Linux gave no guarantee
  ...
  No barriers are implied by ``qatomic_read`` and ``qatomic_set`` in either Linux
  or QEMU.
...
- atomic read-modify-write operations in Linux are of three kinds:
         ...
         ``qatomic_cmpxchg``    returns the old value of the variable
         ===================== =========================================
...
- in QEMU, ``qatomic_read()`` and ``qatomic_set()`` do not participate in
  ...
  is correct in Linux but not in QEMU:
      ...
      | ::                               | ::                             |
      |                                  |                                |
      |   a = qatomic_fetch_add(&x, 2);   |   a = qatomic_fetch_add(&x, 2); |
